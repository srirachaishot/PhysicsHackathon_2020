...
def emission_lines(height,intensity,distance) :   #height minimum of the peaks, distance miminal between peaks
    
    peaks_emission=find_peaks(intensity,height=height,distance=distance)  
    
    return peaks_emission
...
plt.plot(wavelenght_PN,intensity_PN)
for i in peaks_emission_wavelenght_PN : 
    plt.axvline(wavelenght_PN[i],color='r',linestyle=':',label='Emission peaks')
...
plt.subplot(2,1,2)
plt.plot(wavelenght_GAM,intensity_GAM)
...
with open('peaks_PM-M1 46',mode='w') as fic : 
